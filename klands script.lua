...
pcall(function()
    local old = getgenv and getgenv().OaklandsAdminCleanup
    if old then old() end
end)
pcall(function() CAS:UnbindAction("OaklandsFlyBind") end)
pcall(function() CAS:UnbindAction("OaklandsVehicleFlyBind") end)
pcall(function() CAS:UnbindAction("OaklandsTeleportBind") end)
...
local Config = {
    ...
    flying = false,
    vFlying = false,
    espWoodEnabled = false,
    espWoodType = "All",
    espWoodColor = rgb(200, 150, 50),
    espWoodAlpha = 0.3,
    espPlayersEnabled = false,
    espPlayerColor = rgb(255, 50, 50),
    espPlayerAlpha = 0.4,
}
...
local checkboxRefs = {}
local connections = {}
local UpdateCheckboxFromConfig
local SetPlayerFly
local SetVehicleFly
local flyUpBind
local flyDownBind
local activeFlyPart = nil
local activeVehicleFlyPart = nil
local activeFlightTweens = {}
local activeFlightAnchors = {}
local activeFlightValues = {}
...
local function DestroyESP(esp)
    ...
    pcall(function() if esp.box and esp.box.Parent then esp.box:Destroy() end end)
end
...
local function EnsureEspContainer()
    if not espContainer or not espContainer.Parent then
        espContainer = mk("ScreenGui", {
            Name = "WoodESPContainer",
            ResetOnSpawn = false,
        ...
        if not espContainer.Parent then espContainer.Parent = LP:WaitForChild("PlayerGui") end
    end
    return espContainer
end

local function FindAllTreesOnce()
    local trees = {}
    local function scan(parent)
        for _, child in ipairs(parent:GetChildren()) do
            if child.Name == "Tree" or child.Name == "LooseTree" then
...
local function UpdateWoodESP()
    if not Config.espWoodEnabled then
        for _, esp in pairs(woodEsps) do
            DestroyESP(esp)
        end
        woodEsps = {}
        woodEspLabels = {}
    ...
    local now = tick()
    if now - lastWoodESPTime > 3 or next(woodTreeCache) == nil then
        woodTreeCache = FindAllTreesOnce()
        lastWoodESPTime = now
    end

    local container = EnsureEspContainer()

    for _, tree in ipairs(woodTreeCache) do
        if not tree.Parent then
            DestroyESP(woodEsps[tree])
            woodEsps[tree] = nil
        else
            local isLoose = tree.Name == "LooseTree"
            local altName = tree:GetAttribute("AltName") or "Wood"
            local shouldShow = true

            if Config.espWoodType == "Standing" and isLoose then
                shouldShow = false
            elseif Config.espWoodType == "Fallen" and not isLoose then
                shouldShow = false
            end

            if not shouldShow then
                if woodEsps[tree] then
                    DestroyESP(woodEsps[tree])
                    woodEsps[tree] = nil
                end
            else
                local part = GetFirstPart(tree)
                if part then
                    if not woodEsps[tree] then
                        local highlight = mk("Highlight", {
                            Name = "WoodHighlight",
                            Adornee = tree,
                            FillColor = Config.espWoodColor,
                            OutlineColor = Config.espWoodColor,
                            FillTransparency = Config.espWoodAlpha,
                        ...
                        local nameLabel = mk("TextLabel", {
                            ...
                            Text = altName,
                            TextColor3 = Config.espWoodColor,
                            TextSize = 12,
                        ...
                        woodEsps[tree] = {highlight = highlight, billboard = billboard, nameLabel = nameLabel, part = part}
                    else
                        local esp = woodEsps[tree]
                        if esp.highlight and esp.nameLabel then
                            esp.highlight.Adornee = tree
                            esp.highlight.FillColor = Config.espWoodColor
                            esp.highlight.OutlineColor = Config.espWoodColor
                            esp.highlight.FillTransparency = Config.espWoodAlpha
                            esp.billboard.Adornee = part
                            esp.nameLabel.Text = tree:GetAttribute("AltName") or "Wood"
                            esp.nameLabel.TextColor3 = Config.espWoodColor
                        end
                    end
                elseif woodEsps[tree] then
                    DestroyESP(woodEsps[tree])
                    woodEsps[tree] = nil
                end
            end
    ...
    local toRemove = {}
    for tree, esp in pairs(woodEsps) do
        if not tree.Parent or not esp.highlight or not esp.highlight.Parent then
...
local function UpdatePlayerESP()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP and player.Character then
            ...
            if hrp and humanoid then
                playersToShow[player] = true
                if not playerEsps[player] then
                    local highlight = mk("Highlight", {
                        Name = "PlayerHighlight",
                        Adornee = char,
                        FillColor = Config.espPlayerColor,
                        OutlineColor = Config.espPlayerColor,
                        FillTransparency = Config.espPlayerAlpha,
                        OutlineTransparency = 0,
                        DepthMode = Enum.HighlightDepthMode.AlwaysOnTop,
                        Parent = container,
                    })
                    ...
                    local nameLabel = mk("TextLabel", {
                        ...
                        BackgroundTransparency = 1,
                        Text = player.Name,
                        TextColor3 = Config.espPlayerColor,
                        TextSize = 12,
                        Font = Enum.Font.GothamBold,
                        BorderSizePixel = 0,
                        Parent = billboard,
                    ...
                    local healthLabel = mk("TextLabel", {
                        Size = dim2(1, 0, 0, 18),
                    ...
                    playerEsps[player] = {highlight = highlight, billboard = billboard, nameLabel = nameLabel, healthLabel = healthLabel, hrp = hrp, humanoid = humanoid}
                else
                    local esp = playerEsps[player]
                    if esp.highlight and esp.hrp then
                        esp.highlight.Adornee = char
                        esp.highlight.FillColor = Config.espPlayerColor
                        esp.highlight.OutlineColor = Config.espPlayerColor
                        esp.highlight.FillTransparency = Config.espPlayerAlpha
                        esp.billboard.Adornee = hrp
                        esp.nameLabel.Text = player.Name
                        esp.nameLabel.TextColor3 = Config.espPlayerColor
                        esp.healthLabel.Text = math.floor(humanoid.Health) .. " HP"
                        esp.hrp = hrp
    ...
    for player, esp in pairs(playerEsps) do
        if not playersToShow[player] or not esp.highlight or not esp.highlight.Parent then
            table.insert(toRemove, player)
        end
    end
    for _, player in ipairs(toRemove) do
        DestroyESP(playerEsps[player])
        playerEsps[player] = nil
    end
end


local function SafeClickTP()
    if LP.Character then
        local targetPos = Mouse.Hit.Position + Vector3.new(0, 3, 0)
        local hrp = LP.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            local distance = (targetPos - hrp.Position).Magnitude
            local duration = math.max(distance * Config.tpSpeed, 0.1)
            TweenService:Create(hrp, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = CFrame.new(targetPos)}):Play()
        end
    end
end

local function IsFlightKeyDown(key)
...
local function GetMoveDirection()
    ...
    local upKey = flyUpBind and flyUpBind:Get() or Config.flyUpKey
    local downKey = flyDownBind and flyDownBind:Get() or Config.flyDownKey

    if UIS:IsKeyDown(Enum.KeyCode.W) then moveDir = moveDir + camera.CFrame.LookVector end
    if UIS:IsKeyDown(Enum.KeyCode.A) then moveDir = moveDir - camera.CFrame.RightVector end
    if UIS:IsKeyDown(Enum.KeyCode.S) then moveDir = moveDir - camera.CFrame.LookVector end
    if UIS:IsKeyDown(Enum.KeyCode.D) then moveDir = moveDir + camera.CFrame.RightVector end
    if IsFlightKeyDown(upKey) then moveDir = moveDir + Vector3.new(0, 1, 0) end
    if IsFlightKeyDown(downKey) then moveDir = moveDir - Vector3.new(0, 1, 0) end
...
local function GetFlightCFrame(target)
    if not target then return nil end
    if target:IsA("Model") then return target:GetPivot() end
    return target.CFrame
end

local function SetFlightCFrame(target, cf)
    if not target or not cf then return end
...
local function GetFlightParts(target)
...
local function ApplyFlightTween(target, speed, dt)
    if not target then return end
    dt = math.clamp(dt or 0.016, 0.01, 0.05)
    local moveDir = GetMoveDirection()
    local currentCFrame = GetFlightCFrame(target)
    if not currentCFrame then return end
    local targetCFrame = currentCFrame + (moveDir * speed * dt)

    if activeFlightTweens[target] then
        activeFlightTweens[target]:Cancel()
    end
    if activeFlightValues[target] then
        activeFlightValues[target]:Destroy()
    end

    if not activeFlightAnchors[target] then activeFlightAnchors[target] = {} end
    for _, part in ipairs(GetFlightParts(target)) do
        if activeFlightAnchors[target][part] == nil then
            activeFlightAnchors[target][part] = part.Anchored
        end
        part.Anchored = true
    ...
    cfValue:GetPropertyChangedSignal("Value"):Connect(function()
        SetFlightCFrame(target, cfValue.Value)
    end)

    local tween = TweenService:Create(cfValue, TweenInfo.new(dt, Enum.EasingStyle.Linear), {Value = targetCFrame})
    activeFlightTweens[target] = tween
    activeFlightValues[target] = cfValue
    tween:Play()
end

local function StopFlightTween(target)
    if not target then return end
    if activeFlightTweens[target] then
        activeFlightTweens[target]:Cancel()
        activeFlightTweens[target] = nil
    end
    if activeFlightValues[target] then
        activeFlightValues[target]:Destroy()
        activeFlightValues[target] = nil
    end
    if activeFlightAnchors[target] then
        for part, wasAnchored in pairs(activeFlightAnchors[target]) do
        ...
        activeFlightAnchors[target] = nil
    end
end
...
local function CountVehicleSeats(model)
    local count = 0
...
local function GetVehicleRoot(seat)
    ...
    while model and model.Parent and model.Parent:IsA("Model") and CountVehicleSeats(model.Parent) <= 1 do
        model = model.Parent
    end
...
local function UpdateFly(dt)
    if not Config.flying or not LP.Character then return end
    if GetVehicleSeat() then
        SetPlayerFly(false)
        return
    end
    ...
    if activeFlyPart and activeFlyPart ~= hrp then
        StopFlightTween(activeFlyPart)
    end
    activeFlyPart = hrp
    ApplyFlightTween(hrp, Config.flySpeed, dt)
end

local function UpdateCarFly(dt)
    if not Config.vFlying or not LP.Character then return end
    local seat = GetVehicleSeat()
    if not seat then
        StopFlightTween(activeVehicleFlyPart)
        activeVehicleFlyPart = nil
        Config.vFlying = false
        UpdateCheckboxFromConfig("Vehicle Fly", false, true)
        return
    end
    local vehicleRoot = GetVehicleRoot(seat)
    if activeVehicleFlyPart and activeVehicleFlyPart ~= vehicleRoot then
        StopFlightTween(activeVehicleFlyPart)
    end
    activeVehicleFlyPart = vehicleRoot
    ApplyFlightTween(vehicleRoot, Config.carFlySpeed, dt)
end
...
local function Cleanup()
    ...
    activeVehicleFlyPart = nil
    if LP.Character then
        StopFlightTween(LP.Character:FindFirstChild("HumanoidRootPart"))
    end
    StopFlightTween(GetVehicleRoot(GetVehicleSeat()))
    pcall(function() game:GetService("CoreGui"):FindFirstChild("OaklandsUI"):Destroy() end)
    Config.flying = false
    Config.vFlying = false
    Config.espWoodEnabled = false
    ...
    connections = {}
    for _, adorn in pairs(woodEsps) do
        DestroyESP(adorn)
    end
    woodEsps = {}
    for _, adorn in pairs(playerEsps) do
        DestroyESP(adorn)
    end
    playerEsps = {}
    for _, lbl in pairs(playerEspLabels) do
        pcall(function() lbl:Destroy() end)
    end
    playerEspLabels = {}
    pcall(function() if espContainer then espContainer:Destroy() end end)
    espContainer = nil
end

pcall(function()
    if getgenv then
        getgenv().OaklandsAdminCleanup = Cleanup
    end
end)


SetPlayerFly = function(v)
    if v and GetVehicleSeat() then
        Config.flying = false
        UpdateCheckboxFromConfig("Fly", false, true)
        return
    end

    Config.flying = v
    if v and Config.vFlying then
        Config.vFlying = false
        UpdateCheckboxFromConfig("Vehicle Fly", false, true)
        StopFlightTween(activeVehicleFlyPart)
        activeVehicleFlyPart = nil
...
SetVehicleFly = function(v)
    local seat = GetVehicleSeat()
    if v and not seat then
        Config.vFlying = false
        UpdateCheckboxFromConfig("Vehicle Fly", false, true)
        return
    end
    ...
    if not v then
        StopFlightTween(activeVehicleFlyPart)
        activeVehicleFlyPart = nil
        StopFlightTween(GetVehicleRoot(seat))
    end
end
...
Win = Library:CreateWindow({
    Title       = "Suiratcal Client",
    AccentColor = Color3.fromRGB(110, 28, 195),
    Key         = Enum.KeyCode.Insert,
    ConfigName  = "Default",
    BuildType   = "Dev",
    LogoImage   = "rbxassetid://118298630077545",
    DefaultFont = "GothamBold",
})

local function TrackToggle(name, toggle)
    table.insert(checkboxRefs, {name = name, obj = toggle})
    return toggle
end

UpdateCheckboxFromConfig = function(checkboxName, newValue, silent)
    for _, ref in ipairs(checkboxRefs) do
        if ref.name == checkboxName then
            ref.obj:Set(newValue)
            break
        end
    end
end

local MovementCat = Win:AddCategory("Movement")

MovementCat:AddSlider({
    Name = "Fly Speed",
    Min = 10,
    Max = 5000,
...
TrackToggle("Fly", MovementCat:AddToggle({
    Name = "Fly",
    Default = Config.flying,
    Callback = SetPlayerFly,
}))

MovementCat:AddKeybind({
    Name = "Fly Keybind",
    Default = Enum.KeyCode.Q,
    Callback = function()
        SetPlayerFly(not Config.flying)
        UpdateCheckboxFromConfig("Fly", Config.flying, true)
    end,
})

flyUpBind = MovementCat:AddKeybind({
    Name = "Fly Up Keybind",
    Default = Config.flyUpKey,
    Callback = function() end,
})

flyDownBind = MovementCat:AddKeybind({
    Name = "Fly Down Keybind",
    Default = Config.flyDownKey,
    Callback = function() end,
})

MovementCat:AddSeparator()
...
MovementCat:AddSlider({
    ...
    Step = 1,
    Callback = function(v) Config.carFlySpeed = v end,
})

TrackToggle("Vehicle Fly", MovementCat:AddToggle({
    Name = "Vehicle Fly",
    Default = Config.vFlying,
    Callback = SetVehicleFly,
}))
...
MovementCat:AddKeybind({
    ...
    Callback = function()
        SetVehicleFly(not Config.vFlying)
        UpdateCheckboxFromConfig("Vehicle Fly", Config.vFlying, true)
    end,
})
...
MovementCat:AddSlider({
    ...
    Min = 0.0001,
    Max = 1,
    Default = Config.tpSpeed,
    Step = 0.0001,
    Callback = function(v) Config.tpSpeed = v end,
})

local tpEnabled = false
TrackToggle("Click to Teleport", MovementCat:AddToggle({
    Name = "Click to Teleport",
    Default = false,
    Callback = function(v) tpEnabled = v end,
}))
...
MovementCat:AddKeybind({
    ...
    Callback = function()
        if tpEnabled then SafeClickTP() end
    end,
})

local ESPCat = Win:AddCategory("ESP")

TrackToggle("Wood ESP", ESPCat:AddToggle({
    Name = "Wood ESP",
    Default = Config.espWoodEnabled,
    Callback = function(v) Config.espWoodEnabled = v end,
}))

ESPCat:AddColorPicker({
    Name = "Wood ESP Color",
    Default = Config.espWoodColor,
    Callback = function(v)
        Config.espWoodColor = v
    end,
})

ESPCat:AddSlider({
    Name = "Wood Transparency",
    Min = 0,
    Max = 1,
    Default = Config.espWoodAlpha,
    Step = 0.01,
    Callback = function(v) Config.espWoodAlpha = v end,
})

ESPCat:AddSeparator()

TrackToggle("Player ESP", ESPCat:AddToggle({
    Name = "Player ESP",
    Default = Config.espPlayersEnabled,
    Callback = function(v) Config.espPlayersEnabled = v end,
}))
...
ESPCat:AddSlider({
    ...
    Min = 0,
    Max = 1,
    Default = Config.espPlayerAlpha,
    Step = 0.01,
    Callback = function(v) Config.espPlayerAlpha = v end,
})
